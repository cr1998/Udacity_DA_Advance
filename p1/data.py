"""
Created on Wed Nov 30 20:08:40 2016
...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 30 22:02:47 2016
    ...
    if element.tag == 'node':
        ...
        for child in element.findall('tag'):
            if PROBLEMCHARS.match(child.attrib['k']):
                continue
            tagdi = {}
            tagdi['id'] = element.attrib['id']
            key = child.attrib['k']
            if key.find(':') != -1:
                tagdi['type'] = key[0:key.index(':')]
                tagdi['key'] = key[key.index(':') + 1:]
            else:
                tagdi['type'] = 'regular'
                tagdi['key'] = key
            tagdi['value'] = child.attrib['v']
            tags.append(tagdi)
        ...
        start = 0
        for childn in element.findall('nd'):
            wid = element.attrib['id']
            ndic = {}
            ndic['id'] = wid
            ndic['node_id'] = childn.attrib['ref']
            ndic['position'] = start
            start += 1
